...
type BatchPreCheckFunc func(types.Txs) error

// PostCheckFunc is an optional filter executed after CheckTx and rejects
// transaction if false is returned. An example would be to ensure a
// transaction doesn't require more gas than available for the block.
type PostCheckFunc func(types.Tx, *abci.ResponseCheckTx) error
type BatchPostCheckFunc func(types.Txs, *abci.ResponseCheckTx) error
...
// PreCheckMaxBytes checks that the size of the transaction is smaller or equal to the expected maxBytes.
func BatchPreCheckMaxBytes(maxBytes int64) BatchPreCheckFunc {
	return func(txs types.Txs) error {
		txSize := types.ComputeProtoSizeForTxs(txs)
...
func BatchPostCheckMaxGas(maxGas int64) BatchPostCheckFunc {
	return func(txs types.Txs, res *abci.ResponseCheckTx) error {
		return nil
	}
}
